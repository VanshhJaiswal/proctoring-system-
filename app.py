...
FRAME_INTERVAL = 5  # seconds between snapshots

if "last_run" not in st.session_state:
    st.session_state.last_run = 0

current_time = time.time()
if current_time - st.session_state.last_run > FRAME_INTERVAL:
    st.session_state.last_run = current_time
    st.experimental_rerun()

uploaded_image = st.camera_input("📸 Take a snapshot for analysis", key="live_cam")
